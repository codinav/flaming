...
/**
 * Tiptap-based rich-text editor. Keeps a hidden input in sync with the editor
 * HTML so it submits inside the existing server-action forms unchanged.
 */
export function RichTextEditor({
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "prose-fill min-h-[260px] max-w-none px-4 py-4 focus:outline-none",
      },
    },
...
function Toolbar({ editor }: { editor: Editor | null }) {
  if (!editor) return <div className="h-11 border-b border-ink/8 bg-canvas-soft" />;
  ...
  const Btn = ({
    ...
    <button
      ...
      title={label}
      className={cn(
        "flex h-8 w-8 items-center justify-center rounded-lg text-ink-700 transition hover:bg-white",
  ...
  return (
    <div className="flex flex-wrap items-center gap-1 border-b border-ink/8 bg-canvas-soft p-1.5">
